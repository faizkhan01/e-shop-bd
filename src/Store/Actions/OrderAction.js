...
export const fetchOrders = (pageNo = 1, orderType) => {
  return (dispatch) => {
    ...
    axios
      .get(`http://localhost:9000/orders/${orderType}?pageNo=${pageNo}`)
      .then((response) => {
        dispatch(
          fetchSuccessAction({
...
export const fetchuserOrders = (userId) => {
  return (dispatch) => {
    dispatch(fetchOrderDataAction);
    axios
      .get(`http://localhost:9000/orders/user-orders/${userId}`)
      .then((response) => {
...
export const fetchOrdersByDate = (date) => {
  return (dispatch) => {
    dispatch(fetchOrderDataAction);
    axios
      .get(`http://localhost:9000/orders//daily-orders/${"" + date}`)
      .then((response) => {
        dispatch(userOrderAction(response.data));
      })
      .catch((err) => {
...
export const makeOrder = (orderData) => {
  return (dispatch) => {
    dispatch(fetchOrderDataAction);
    axios
      .post(`http://localhost:9000/orders/make-order`, orderData)
      .then((response) => {
...
export const updateOrderStatus = (orderInfo) => {
  return (dispatch) => {
    dispatch(fetchOrderDataAction);
    axios
      .patch(`http://localhost:9000/orders/update-status`, orderInfo)
      .then((response) => {
        dispatch(makeOrderAction());
      })
      .catch((err) => {
