...
export const fetchUsers = (pageNumber = 1) => {
  return (dispatch) => {
    ...
    axios
      .get(`http://localhost:9000/users?pageNo=${pageNumber}`)
      .then((response) => {
        dispatch(
...
export const createUser = (userInfo, history) => {
  ...
  return (dispatch) => {
    dispatch(fetchUserDataAction);
    axios
      .post("http://localhost:9000/users/create-user", user)
      .then((response) => {
...
export const updateUser = (userInfo) => {
  return (dispatch) => {
    dispatch(fetchUserDataAction);
    axios
      .patch(`http://localhost:9000/users/update-user`, userInfo)
...
export const deleteUser = (userId) => {
  return (dispatch) => {
    dispatch(fetchUserDataAction);
    axios
      .delete(`http://localhost:9000/users/delete-user/${userId}`)
      .then((response) => {
